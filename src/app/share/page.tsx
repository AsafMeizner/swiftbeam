    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-green-50/30 p-4 md:p-8">
      <div className="max-w-5xl mx-auto">
        ...
        <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="mb-8">
          <h1 className="text-3xl md:text-4xl font-bold text-gray-900 mb-2">Share Files</h1>
          <p className="text-gray-600 text-lg">Drag, drop, and share files instantly with nearby devices</p>
        </motion.div>
        ...
        <div className="grid lg:grid-cols-3 gap-8">
                ...
                <Card className="border-0 shadow-lg bg-white/90 backdrop-blur-sm">
                  <CardContent className="p-6">
                    <div className="flex items-center justify-between mb-6">
                      <h3 className="text-lg font-semibold text-gray-900">Ready to Share ({files.length})</h3>
                      <div className="text-sm text-gray-600">Total: {formatFileSize(totalSize)}</div>
                    </div>
          ...
          <div className="space-y-6">
            ...
            <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.2 }}>
              <Card className="border-0 shadow-lg bg-white/90 backdrop-blur-sm">
                <CardContent className="p-6">
                  <h3 className="text-lg font-semibold text-gray-900 mb-4">Transfer Control</h3>

                  <div className="space-y-4">
                    <div className="text-sm text-gray-600">
                      {files.length} file{files.length !== 1 ? "s" : ""} ready
                      {selectedDevice && (
                        <div className="mt-1 font-medium text-gray-900">→ {selectedDevice.name}</div>
                      )}
                    </div>
